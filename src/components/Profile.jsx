...
export default function Profile({ onSwitch }) {
  ...
  const discoHexColors = [
    '#4c1d95', '#1e3a8a', '#064e3b', '#991b1b', '#312e81', '#713f12', '#831843', '#155e75', '#064e3b', '#9a3412'
  ]
  ...
  useEffect(() => {
    let themeColor
    if (isDiscoMode) {
      themeColor = discoHexColors[currentColorIndex]
    } else {
      themeColor = isDark ? '#000000' : '#f9fafb'
    }
  ...
  }, [isDark, isDiscoMode, currentColorIndex])
  ...
  return (
    <div className={`min-h-screen transition-all duration-500 ${getBackgroundClass()} ${isDark && !isDiscoMode ? 'text-gray-100' : isDiscoMode ? 'text-white' : 'text-gray-900'
      }`}>

      {/* Glass Header */}
      <header
        className={`sticky top-0 z-50 backdrop-blur-md border-b transition-all duration-300 ${isDark || isDiscoMode
      ...
      >
        <div className="max-w-5xl mx-auto px-4 py-3 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
          {/* Name */}
          <h1
            className={`text-lg sm:text-xl font-semibold transition-colors duration-300 ${isDark || isDiscoMode ? 'text-white' : 'text-gray-900'
              }`}
          >
            Dinesh Kannaujiya
          </h1>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <button
              onClick={toggleDisco}
              className={`px-3 py-2 rounded-full backdrop-blur-sm border transition-all duration-300 hover:scale-105 text-xs sm:text-sm font-medium ${isDiscoMode
            ...
            <button
              onClick={toggleTheme}
              className={`p-2 sm:p-3 rounded-full backdrop-blur-sm border transition-all duration-300 hover:scale-110 ${isDark || isDiscoMode
      ...
      <div className="p-4 pt-8">
        <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          {/* Left card */}
          <aside className={`md:col-span-1 backdrop-blur-sm rounded-2xl p-6 border transition-all duration-300 ${isDark || isDiscoMode
              ? 'bg-white/5 border-white/10'
              : 'bg-white/60 border-gray-200/50 shadow-lg'
            }`}>
            <div className="flex items-center gap-4">
              <div className={`w-16 h-16 rounded-full backdrop-blur-sm border flex items-center justify-center text-lg font-semibold transition-all duration-300 ${isDark || isDiscoMode
              ...
              <div>
                ...
                <p className={`text-sm transition-colors duration-300 ${isDark || isDiscoMode ? 'text-gray-400' : 'text-gray-600'
            ...
            <div className="mt-6 space-y-3 text-sm">
              <div>
                <p className={`text-xs uppercase transition-colors duration-300 ${isDark || isDiscoMode ? 'text-gray-500' : 'text-gray-500'
                  }`}>Contact</p>
                <div className="flex gap-3 mt-2">
                  {socials.map((s) => (
                    <a key={s.name} href={s.url} target="_blank" rel="noreferrer"
                      className={`flex items-center gap-2 transition-colors duration-300 ${isDark || isDiscoMode
              ...
              <div>
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  {['React Native', 'React.js', 'Node.js', 'Express', 'MongoDB', 'Tailwind CSS', 'Android', 'Firebase', 'Git'].map(s => (
                    <span key={s} className={`text-xs px-2 py-1 rounded backdrop-blur-sm border transition-all duration-300 ${isDark || isDiscoMode
                        ? 'bg-white/5 border-white/10'
              ...
              <div>
                ...
                <p className={`text-sm transition-colors duration-300 ${isDark || isDiscoMode ? 'text-gray-300' : 'text-gray-700'
          ...
          <main className="md:col-span-2 flex flex-col gap-6">
            <section className={`backdrop-blur-sm rounded-2xl p-6 border transition-all duration-300 ${isDark || isDiscoMode
                ? 'bg-white/5 border-white/10'
                : 'bg-white/60 border-gray-200/50 shadow-lg'
              }`}>
              <h2 className="text-lg font-medium">About</h2>
              <p className={`mt-3 text-sm leading-relaxed transition-colors duration-300 ${isDark || isDiscoMode ? 'text-gray-300' : 'text-gray-700'
                }`}>
              ...
              <div className={`mt-3 text-xs transition-colors duration-300 ${isDark || isDiscoMode ? 'text-gray-500' : 'text-gray-500'
                }`}>
            ...
            <section className={`backdrop-blur-sm rounded-2xl p-6 border transition-all duration-300 ${isDark || isDiscoMode
              ...
              <div className="flex items-center justify-between">
                ...
                <span className={`text-xs transition-colors duration-300 ${isDark || isDiscoMode ? 'text-gray-500' : 'text-gray-500'
                  }`}>Full-stack · Mobile</span>
              </div>

              <div className="mt-4 space-y-4">
                {projects.map((p, idx) => (
                  <article key={idx} className={`p-4 rounded-lg backdrop-blur-sm border transition-all duration-300 ${isDark || isDiscoMode
                    ...
                    <div className="flex items-start justify-between">
                      <div>
                        ...
                        <p className={`text-xs mt-1 transition-colors duration-300 ${isDark || isDiscoMode ? 'text-gray-300' : 'text-gray-600'
                          }`}>{p.desc}</p>
                      </div>
            ...
            <section className={`backdrop-blur-sm rounded-2xl p-6 grid grid-cols-1 md:grid-cols-2 gap-6 border transition-all duration-300 ${isDark || isDiscoMode
                ? 'bg-white/5 border-white/10'
                : 'bg-white/60 border-gray-200/50 shadow-lg'
              }`}>
              <div>
                <h2 className="text-lg font-medium">Experience</h2>
                <div className={`mt-3 space-y-3 text-sm transition-colors duration-300 ${isDark || isDiscoMode ? 'text-gray-300' : 'text-gray-700'
                  }`}>
                  <div>
                    <div className="font-semibold">Frontend Developer — SR EDU Technology Pvt Ltd</div>
                    <div className={`text-xs transition-colors duration-300 ${isDark || isDiscoMode ? 'text-gray-400' : 'text-gray-500'
                  ...
                  <div>
                    ...
                    <div className={`text-xs transition-colors duration-300 ${isDark || isDiscoMode ? 'text-gray-400' : 'text-gray-500'
                      }`}>Feb 2025 — Jun 2025 · Gorakhpur</div>
              ...
              <div>
                ...
                <div className={`mt-3 text-sm transition-colors duration-300 ${isDark || isDiscoMode ? 'text-gray-300' : 'text-gray-700'
                  }`}>
                  ...
                  <div className="mt-3">
                    ...
                    <ul className={`list-disc list-inside mt-2 space-y-1 text-sm transition-colors duration-300 ${isDark || isDiscoMode ? 'text-gray-300' : 'text-gray-700'
                      }`}>
            ...
            <section className={`backdrop-blur-sm rounded-2xl p-6 flex items-center justify-between border transition-all duration-300 ${isDark || isDiscoMode
                ? 'bg-white/5 border-white/10'
                : 'bg-white/60 border-gray-200/50 shadow-lg'
              }`}>
              <div>
              ...
              <div className={`text-xs transition-colors duration-300 ${isDark || isDiscoMode ? 'text-gray-500' : 'text-gray-500'
